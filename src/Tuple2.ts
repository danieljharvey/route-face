// credit to https://github.com/Morglod/ts-tuple-hacks/blob/master/index.ts
...
type Shift<A, T extends any[]> = ((
  head: A,
  ...args: T
) => void) extends (...tail: infer U) => any
  ? U
  : never

export type Push<A, T extends any[]> = Reverse<
  Shift<A, Reverse<T>>
>

type p = Push<'dog', ['log', 'slog']>

export const pushTuple = <A, T extends any[]>(
  a: A,
  ...as: T
) => [...as].concat([a]) as Push<A, T>
...
type PipeFunc<T extends any[], V> = (
  ...f: Reverse<Compose<T, V>>
) => (x: V) => Last<T, V>

type IfNotExtends<A, B, True, False> = A extends B
  ? False
  : True
type IfExtends<A, B, True, False> = A extends B
  ? True
  : False
...
> = {
  0: never
  1: Phead extends keyof T
    ? TypeByPath<T[Phead], Tail<Path>>
    : never
...
> = {
  0: never
  1: Phead extends keyof T
    ? PathOf<T[Phead], Tail<Path>, FullPath>
    : never
